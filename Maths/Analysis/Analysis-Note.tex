...
\begin{document}
    ...
    有 $\len{A\mat{x}} \leqslant \len{A}\len{\mat{x}}$，$|A\mat{x}| \leqslant |A||\mat{x}|$
        ...
        进一步地，可定义偏微分。任取一些维度的集合，不妨记 $\mat{x} = (\mat{x}, \mat{y})$，则 $\Dif_x F(\mat{x}_0, \mat{y}_0) := A$。这里把子空间整体视为变量。
    ...
    \section{重积分}
        ...
        \begin{longtable}[h]{c | c | w{c}{10cm}}
            ...
            球面坐标 & $\begin{cases}
                ...
                z = \rho \cos\theta
            \end{cases}$ & $\begin{aligned}
                \dif s &= \rho^2 \sin\varphi \sdif\theta \sdif\varphi \quad (\rho \equiv r) \\
    ...
    $$\begin{aligned}
        S =& \frac{1}{2} \oint_{(L)} y\sdif x + x \sdif y \\
        V =& \frac{1}{3} \oint_{(S)} x\sdif y \land \dif z + y\sdif z \land \dif x + z\sdif x \land \dif y
    \end{aligned}$$
    ...
    \section{常数项级数}
        ...
        也可在 $n \to \infty$ 时对 $a$ 做等价无穷大/小替换，本质仍是比较准则，但没有显式写出 $b$。
    ...
    \section{幂级数}
        \subsection{收敛半径}
        对 $\forall a_n \neq 0$，有
        $$R = \lim_{n \to \infty} \frac{1}{\sqrt[n]{|a_{n}|}} = \lim_{n \to \infty} \left|\frac{a_n}{a_{n + 1}}\right|$$

        $R = 0$ 退化为单点 $\{x_0\}$，$R = +\infty$ 为 $\field{R}$。$0 < R < +\infty$ 时，区间端点的敛散性未知，需另行验证。

        若端点为奇点，则{\heiti 必然不在收敛域}内。但非奇点未必收敛，如 $\frac{1}{1 + x}$ 在 $x = 1$ 发散。在复平面上，收敛半径由最近的奇点决定，这同样会限制 $\field{R}$ 上的收敛半径。
