...
\begin{document}
    ...
    \section{计算}
        ...
        $$\begin{aligned}
            \text{Sin/Cos}_n &= \int_{0}^{\pi / 2} \sin^n\difx = \int_{0}^{\pi / 2} \cos^n\difx \text{ 证明：换元。} \\
    ...
    向量值函数：$f: \Omega \to \R{m}$
        ...
        \subsection{极限与连续}
        多重极限：$\epsilon-\delta$。需限定 $P(x_0, y_0, \cdots)$ 为聚点，否则无法逼近。
    ...
    \section{数量值函数微分}
        ...
        \begin{tikzpicture}
            \node (A) at (-5, 0) [rectangle, draw = black]{偏导存在且连续};
            \node (B) at (0, 0) [rectangle, draw = black]{可微};
            \node (C) at (60: 2) [rectangle, draw = black]{连续};
            \node (D) at (0: 5) [rectangle, draw = black]{偏导存在};
            \node (E) at (-60: 2) [rectangle, draw = black]{方向导数存在};
    ...
    \section{低维微分几何}
        ...
        这是向量的混和积，于是有行列式表示 $\begin{vmatrix}
            (x - x_0) & (y - y_0) & (z - z_0) \\ 
            \frac{\partial x}{\partial u} (u_0, v_0) & \frac{\partial y}{\partial u} (u_0, v_0) & \frac{\partial z}{\partial u} (u_0, v_0) \\
            \frac{\partial x}{\partial v} (u_0, v_0) & \frac{\partial y}{\partial v} (u_0, v_0) & \frac{\partial z}{\partial v} (u_0, v_0)
        \end{vmatrix} = 0$。
        按行展开，得法线方程：
        ...
        特别地：显式表示 $z = z(x, y)$，视为 $u = x, v = y, \mat{r} = (x, y, z(x, y))$ \\
